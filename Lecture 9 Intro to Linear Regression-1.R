...
on_my_own_model <- lm(runs ~ hits, data = mlb11)
plot(mlb11$runs ~ mlb11$hits)
abline(on_my_own_model)
...
batting_avg_model <- lm(formula = runs ~ bat_avg, data = mlb11)
summary(batting_avg_model)
...
qqline(obs_model$residuals)  # adds diagonal line to the normal prob plot
