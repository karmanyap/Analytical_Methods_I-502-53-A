...
kobe[1:50,3:6]
  ...
  #  dim(kobe_streak)
 # kobe$kobe_streak <- calc_streak(kobe$basket)
  
  
##  Lets see how this looks...
kobe_streak  
table(kobe_streak)
    barplot(table(kobe_streak), main = "Shooting Streak", xlab = "Streak Length", ylab = "Count")
?barplot    
    table(kobe$basket)/133
library(e1071)
    skewness(kobe_streak)
?skewness    
...
##   read more on the sample function in help.  ?sample()
       sim_unfair_coin_1 <- sample(outcomes, size = 100, replace = TRUE, prob = c(0.2, 0.8))
       sim_unfair_coin_1
       table(sim_unfair_coin_1)
       table(sim_unfair_coin)/1000
...
##      Using  calc_streak , compute the streak lengths of  sim_basket .   
      ...
      table(calc_streak(sim_basket))
       barplot(table(calc_streak(sim_basket)), main = "Simulated Shooting Streak", xlab = "Streak Length", ylab = "Count")
...
#outcomes_new <- c("H", "M")
#sim_basket_new <- sample(outcomes, size = 133, replace = TRUE, prob = c(.45, 0.55))
#sim_basket_new
#table(sim_basket_new)
#table(calc_streak(sim_basket))
#table(calc_streak(sim_basket_new))
