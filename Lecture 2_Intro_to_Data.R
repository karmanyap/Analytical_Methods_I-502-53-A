...
table(cdc$gender)
summary(cdc$gender)
malecount<-sum(cdc$gender=='m')
sum(cdc$gender=='m'&cdc$genhlth=='excellent')
maleexchlth <- sum(cdc$gender=='m'&cdc$genhlth=='excellent')
maleexchlth
...
cor.test(cdc$weight , cdc$wtdesire )
?cor.test
...
?class
?typeof
#Ans: wdiff is an integer. If an observation of wdiff is 0, this means that the person has reached their desired weight.
wdiff
#   What if wdiff is positive or negative?

#Ans: If wdiff is positive, the desired weight is higher than the current weight. 
#Ans: Additionally, if wdiff is negative, the desired weight is less than the current weight.

#.	Describe the distribution of wdiff in terms of its center, shape, and spread, including any plots you use.
plot(wdiff, xlab = "Count", ylab = "Diff. btwn wtdesire and weight")
boxplot(wdiff, ylab= "Diff. btwn wtdesire and weight")
boxplot(wdiff)
hist(wdiff)
hist(wdiff, breaks = 100, las = 1)
summary(wdiff)
...
#summary(cdc$weight, cdc$gender)
summary(cdc$weight)
summary(wdiff)
summary(cdc$gender)
m_or_over30 <- subset(cdc, gender == "m" | age > 30)
summary(subset(cdc,gender=='m'))
summary(subset(cdc,gender=='f'))

boxplot(wdiff ~ cdc$gender, xlab = "Gender", ylab = "Diff. btwn wtdesire and weight")
...
mean(cdc$weight)
sd(cdc$weight)
onesd1 <- cdc$weight[abs(scale(cdc$weight)) <1]
head(onesd1)
onesd1
typeof(onesd1)
length(onesd1)
wt_ratio <- length(onesd1)/length(cdc$weight)
wt_ratio
table(onesd1)
wt<- cdc$weight
onesd = wt[abs(wt - mean(wt)) < sd(wt)]
onesd
summary(onesd)
dim(onesd)
...
manya <- cdc$weight[abs(scale(cdc$weight)) <1]
head(manya)
manya
typeof(manya)
length(manya)
wt_ratio_1 <- length(manya)/length(cdc$weight)
wt_ratio_1
table(manya)
?IQR
IQR(cdc$weight)
...
onesdwdiff <- wdiff[abs(scale(wdiff)) <1]
head(onesdwdiff)
onesdwdiff
typeof(onesdwdiff)
length(onesdwdiff)
wdiff_ratio <- length(onesdwdiff)/length(wdiff)
wdiff_ratio
table(onesdwdiff)
